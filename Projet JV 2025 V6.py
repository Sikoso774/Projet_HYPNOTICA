...
def presentation(images, duree_affichage=3000):
    """
    Affiche une séquence d'images avec un fondu enchaîné.
    ...
    police_ENSEA = pygame.font.Font("assets/fonts/MINDCONTROL.ttf", 30)

    texte_nom = police_nom.render("Zoléni KOKOLO ZASSI", True, blanc)
    texte_nom_rect = texte_nom.get_rect(center =(largeur // 1.5, hauteur // 1.5 + 100))

    texte_ENSEA = police_ENSEA.render("Projet Hypnotica", True, blanc)
    texte_ENSEA_rect = texte_ENSEA.get_rect(topleft = (0,0))
    ...
    for image_path in images:
        ...
        while pygame.time.get_ticks() - temps_debut < duree_affichage:
            ...
            if temps_ecoule < duree_affichage / 2:
                alpha = int(temps_ecoule * alpha_vitesse)
            else:
                alpha = int((duree_affichage - temps_ecoule) * alpha_vitesse)

            # Application de l'alpha à l'image et au texte
            image.set_alpha(alpha)
            texte_nom.set_alpha(alpha)
            texte_ENSEA.set_alpha(alpha)
